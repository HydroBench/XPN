...
!	use don_interaction
	...
	double precision::S1,S2
       ...
       NNMC=NMB
       RESP=0.D0
ccccccc BOUCLE I UV(ij) UV(kl)   +UV VU
                  RES1A=0.D0
                  DO IA=1,NMA
                     IA1=IA+NPA
...
CCCCCCCCCCc BOUCLE   VU UV + VU VU en enlevant les termes VU UV
                        DO IC=1,NMC         !  c-
                           IC1=IC+NPC
                           ICC=IC1+ITRJU
                     RES1C=0.D0
...
c--------------------------------------------------------------------------------------------------
                  ...
                  END DO   !a+ donc c+
CCCCCCCCCc fin boucle VU  UV +  VU VU   III + IV   
CCCCCCCCCCCCCC TERMES DE PAIRINGCCCCCCCCCCCCCCCCCCCCCC pairing si meme isospin
cccccc BOUCLE V UU(ij) UU(kl)
...
c--------------------------------------------------------------------------------------------------------
                  ...
                  END DO
CCCCCCCCCC fin de la boucle UU UU  
cccccc BOUCLE VI Vabar Vbbar(ij) Vcbar Vdbar(kl)   <"kl"|v|IJ> =..= <lk||ij>::::<cd|v|ab>
c                  res1V=0.D0
...
c-------------------------------------------------------------------------------------
		     ...
		     RESP=RESP+RES1B*XPASV(IAV,I,1)
                  END DO
CCCCCCCCCC fin de la boucle VV VV
CCCCCCCCCFIN TERME DE PAIRINGCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
         RES1A=RES1A-RESP  
         RESC_pn=RES1A
	END
